...
class StockCrossOverSignalTest(unittest.TestCase):
	...
	def test_generate_timestamp_returns_consecutive_dates(self):
		...
		expected = [
			datetime(2014, 2, 3), datetime(2014, 2, 4),
			datetime(2014, 2, 5), datetime(2014, 2, 6),
			datetime(2014, 2, 7), datetime(2014, 2, 8),
			datetime(2014, 2, 9), datetime(2014, 2, 10),
			datetime(2014, 2, 11), datetime(2014, 2, 12),
			datetime(2014, 2, 13)]
	...
	def test_stock_with_less_data_returns_neutral(self):
		"""Even though the series has a downward crossover, we return
		neutral because there are not enough data points"""
		date_to_check = datetime(2014, 2, 13)
		self.given_a_series_of_prices([
	...
	def test_stock_with_downard_crossover_returns_sell(self):
		date_to_check = datetime(2014, 2, 13)
		self.given_a_series_of_prices([
	...
	def test_stock_with_upward_crossover_returns_buy(self):
		date_to_check = datetime(2014, 2, 13)
		self.given_a_series_of_prices([
	...
	def test_should_have_11_days_worth_of_data(self):
		"""Should return signal even if there is less than 11 number of
		updates as in the case where some days have no updates but we
		pick up the previous closing price to fill in the value"""
		date_to_check = datetime(2014, 2, 13)
		self.given_a_series_of_prices([
	...
	def test_date_to_check_can_be_beyond_last_update_date(self):
		"""We have updates upto 13th, but we are checking signal on
		15th. It should just fill in the values for 14th and 15th since
		there are no updates on these days"""
