...
class EmailActionTest(unittest.TestCase):
	def setUp(self):
		self.action = EmailAction(to="pslinge144@gmail.com")
	...
	def test_connection_closed_after_sending_email(self, mock_smtp_class):
	...
	def test_connection_closed_if_send_gives_error(self, mock_smtp_class):
		...
		try:
			self.action.execute("MSFT has crossed $20 price level")
		except Exception:
			pass
		self.assertTrue(mock_smtp.quit.called)
